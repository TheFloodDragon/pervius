        ...
        if ui.is_rect_visible(inner_rect) {
            ...
            if has_focus && let Some(cursor_range) = state.cursor.range(&galley) {
                if cursor_range.is_empty() {
                    ...
                    let has_large_row = galley.rows.iter().any(|r| r.glyphs.len() > 2048);
                    if has_large_row {
                        // Large row: paint selection as overlay to avoid cloning the giant row
                        ...
                        let galley_addr = Arc::as_ptr(&galley) as usize;
                        let cache_hit = state.selection_cache.as_ref().is_some_and(|c| {
                            c.cursor_range == cursor_range && c.source_galley_addr == galley_addr
                        });
                        if cache_hit {
                            galley = Arc::clone(
                                &state.selection_cache.as_ref().unwrap().painted_galley,
                            );
                        } else {
                            paint_text_selection(&mut galley, ui.visuals(), &cursor_range, None);
                            state.selection_cache =
                                Some(super::state::SelectionCache {
                                    cursor_range,
                                    source_galley_addr: galley_addr,
                                    painted_galley: Arc::clone(&galley),
                                });
                        }
                    }
                }
            ...
            // For painting: split large rows into chunks for tessellation culling
            let paint_galley = split_galley_for_painting(
                &galley,
                &mut state.split_galley_cache,
            );
            painter.galley(
...
fn events(
    ...
    for event in &events {
        ...
        if let Some(new_ccursor_range) = did_mutate_text {
            ...
            // For very large text, skip the immediate re-layout to keep the edit frame
            // responsive. The next frame's layouter call will pick up the change.
            if text.as_str().len() <= 100_000 {
...
/// Paint selection background as a simple rectangle overlay, avoiding the
/// expensive `Arc::make_mut` + mesh modification path in `paint_text_selection`.
/// Used for rows with many glyphs where cloning the row data is too costly.
/// Trade-off: selected text color is NOT changed (only background highlight).
fn paint_selection_overlay(
    painter: &crate::Painter,
    galley: &Galley,
    galley_pos: crate::Pos2,
    cursor_range: &text_selection::CCursorRange,
    visuals: &crate::Visuals,
) {
    use crate::pos2;

    let [min, max] = cursor_range.sorted_cursors();
    let min_lc = galley.layout_from_cursor(min);
    let max_lc = galley.layout_from_cursor(max);
    let bg = visuals.selection.bg_fill;

    for ri in min_lc.row..=max_lc.row {
        let placed_row = &galley.rows[ri];
        let row = &*placed_row.row;
        let row_top = galley_pos.y + placed_row.pos.y;
        let row_bottom = row_top + row.size.y;
        let row_x = galley_pos.x + placed_row.pos.x;

        let left = if ri == min_lc.row {
            row_x + row.x_offset(min_lc.column)
        } else {
            row_x
        };
        let right = if ri == max_lc.row {
            row_x + row.x_offset(max_lc.column)
        } else {
            let newline_extra = if placed_row.ends_with_newline {
                row.size.y / 2.0
            } else {
                0.0
            };
            row_x + row.size.x + newline_extra
        };

        painter.rect_filled(
            Rect::from_min_max(pos2(left, row_top), pos2(right, row_bottom)),
            0.0,
            bg,
        );
    }
...
/// Split large rows into smaller chunks so the tessellator's row-level culling
/// can skip non-visible portions. Cached to avoid per-frame overhead.
fn split_galley_for_painting(
    galley: &Arc<Galley>,
    cache: &mut Option<super::state::SplitGalleyCache>,
) -> Arc<Galley> {
    const CHUNK_SIZE: usize = 512;
    const THRESHOLD: usize = 2048;

    let needs_split = galley.rows.iter().any(|pr| pr.glyphs.len() > THRESHOLD);
    if !needs_split {
        *cache = None;
        return Arc::clone(galley);
    }

    let source_addr = Arc::as_ptr(galley) as usize;
    if let Some(c) = cache.as_ref() {
        if c.source_addr == source_addr {
            return Arc::clone(&c.split_galley);
        }
    }

    let split = do_split_galley(galley, CHUNK_SIZE, THRESHOLD);
    let split = Arc::new(split);
    *cache = Some(super::state::SplitGalleyCache {
        source_addr,
        split_galley: Arc::clone(&split),
    });
    split
}

fn do_split_galley(galley: &Galley, chunk_size: usize, threshold: usize) -> Galley {
    use epaint::text::{PlacedRow, RowVisuals};
    use epaint::{Mesh, Rect};
    ...
    for placed_row in &galley.rows {
        let row = &*placed_row.row;
        if row.glyphs.len() <= threshold {
            total_vertices += row.visuals.mesh.vertices.len();
            total_indices += row.visuals.mesh.indices.len();
            new_rows.push(placed_row.clone());
            continue;
        }

        let all_glyphs = &row.glyphs;
        let gvr = &row.visuals.glyph_vertex_range;
        let all_mesh = &row.visuals.mesh;

        // Clone row once, empty heavy fields — cheap base for per-chunk clones
        let mut base_row = (*placed_row.row).clone();
        base_row.glyphs = Vec::new();
        base_row.visuals = RowVisuals::default();

        let num_chunks = all_glyphs.len().div_ceil(chunk_size);
        for chunk_idx in 0..num_chunks {
            let g_start = chunk_idx * chunk_size;
            let g_end = (g_start + chunk_size).min(all_glyphs.len());
            let chunk_glyphs = &all_glyphs[g_start..g_end];

            // Vertex range for this chunk's glyphs
            let v_start = chunk_glyphs[0].first_vertex as usize;
            let v_end = if g_end < all_glyphs.len() {
                all_glyphs[g_end].first_vertex as usize
            } else {
                gvr.end
            };
            let v_count = v_end - v_start;

            let chunk_vertices = all_mesh.vertices[v_start..v_end].to_vec();

            // Regenerate indices: each glyph with ≥4 vertices = 1 quad = 6 indices
            let mut chunk_indices = Vec::with_capacity(chunk_glyphs.len() * 6);
            for (i, g) in chunk_glyphs.iter().enumerate() {
                let next_v = if i + 1 < chunk_glyphs.len() {
                    chunk_glyphs[i + 1].first_vertex as usize
                } else {
                    v_end
                };
                if next_v - g.first_vertex as usize >= 4 {
                    let base = (g.first_vertex as usize - v_start) as u32;
                    chunk_indices.extend_from_slice(&[
                        base, base + 1, base + 2,
            ...
            let remapped_glyphs: Vec<_> = chunk_glyphs.iter().map(|g| {
                let mut g = *g;
                g.first_vertex -= v_start as u32;
                g
            }).collect();

            let mesh_bounds = if chunk_vertices.is_empty() {
                Rect::NOTHING
            } else {
            ...
            let mut chunk_row = base_row.clone();
            chunk_row.glyphs = remapped_glyphs;
            chunk_row.visuals = RowVisuals {
                mesh: Mesh {
                    indices: chunk_indices,
                    vertices: chunk_vertices,
                    texture_id: all_mesh.texture_id,
                },
                mesh_bounds,
                glyph_index_start: 0,
                glyph_vertex_range: 0..v_count,
            };

            total_vertices += chunk_row.visuals.mesh.vertices.len();
            total_indices += chunk_row.visuals.mesh.indices.len();

            let is_last = chunk_idx == num_chunks - 1;
            new_rows.push(PlacedRow {
                pos: placed_row.pos,
                row: Arc::new(chunk_row),
                ends_with_newline: if is_last { placed_row.ends_with_newline } else { false },
            });
        }
    }

    // Clone galley and replace rows (preserves pub(crate) intrinsic_size)
    let mut split = (*galley).clone();
    split.rows = new_rows;
    split.num_vertices = total_vertices;
    split.num_indices = total_indices;
    split
}
