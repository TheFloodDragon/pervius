...
/// Island 样式参数
#[derive(Clone)]
pub struct IslandStyle {
    /// 圆角半径
    pub radius: f32,
    /// 背景填充色
    pub fill: egui::Color32,
    /// 角落遮罩色（窗口底色，用于裁剪溢出的方角内容）
    pub mask: egui::Color32,
}

/// 绘制 island 圆角背景
pub fn paint(ui: &egui::Ui, rect: egui::Rect, style: &IslandStyle) {
    ui.painter().rect_filled(
        rect,
        egui::CornerRadius::same(style.radius as u8),
        style.fill,
    );
}
...
pub fn paint_corner_mask(ui: &egui::Ui, rect: egui::Rect, style: &IslandStyle) {
    let r = style.radius;
    let color = style.mask;
