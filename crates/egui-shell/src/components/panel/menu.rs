...
pub fn menu_item(
    ...
    render_menu_item(ui, theme, label, &shortcut, true)
}
...
pub fn menu_item_if(
    ...
    let shortcut = keybind.map(|k| k.label()).unwrap_or_default();
    render_menu_item(ui, theme, label, &shortcut, enabled)
}

/// 菜单项：label 靠左，快捷键靠右（原始字符串版本，用于系统级快捷键如 Alt+F4）
pub fn menu_item_raw(ui: &mut egui::Ui, theme: &MenuTheme, label: &str, shortcut: &str) -> bool {
    render_menu_item(ui, theme, label, shortcut, true)
}

/// 带 enabled 控制的原始菜单项
pub fn menu_item_raw_if(
    ui: &mut egui::Ui,
    theme: &MenuTheme,
    label: &str,
    shortcut: &str,
...
fn render_menu_item(
    ...
    let disabled_color = egui::Color32::from_rgb(100, 100, 100);
    ...
    let shortcut_galley = if !shortcut.is_empty() {
        Some(
            ui.painter()
                .layout_no_wrap(shortcut.to_owned(), shortcut_font, shortcut_color),
        )
    } else {
        None
    };
    ...
    let (rect, resp) = ui.allocate_at_least(egui::vec2(desired_w, height), sense);
