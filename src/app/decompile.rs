...
impl App {
    /// 检查后台反编译是否完成
    pub(crate) fn check_decompiling(&mut self) {
        ...
        match result {
            Ok(()) => {
                ...
                self.layout.editor.refresh_class_tabs(hash);
                self.toasts
    ...
    /// 轮询后台重反编译启动结果
    pub(crate) fn check_re_decompile(&mut self) {
    ...
    pub(crate) fn check_single_decompile(&mut self) {
